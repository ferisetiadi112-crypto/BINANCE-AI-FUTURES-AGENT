/**
 * Embedded SQLite schema — BINANCE AI FUTURES AGENT
 *
 * This module is the BUNDLED copy of src/backend/database/schema.sql.
 * It exists because the production Nitro build (.output/server/index.mjs)
 * does not copy loose .sql assets, so a runtime readFileSync("schema.sql")
 * fails with ENOENT (_ssr/schema.sql). By keeping the schema as a string
 * constant it is inlined into the server bundle and schema initialization
 * works regardless of process.cwd() or file layout.
 *
 * SOURCE OF TRUTH: src/backend/database/schema.sql
 * schema-source.test.ts guards against drift between the two.
 */
